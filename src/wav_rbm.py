...
import numpy, time, cPickle, gzip, PIL.Image, pylab
...
def test_rbm(learning_rate=0.1, training_epochs = 4, # 15
    ...
    rbm = RBM( input = x, n_visible=freq_rows*timebins_cols,
    ...
    for epoch in xrange(training_epochs):
        ...
        wav_filter_image = tile_raster_images(
        ...
        # pylab.plot(
        image = PIL.Image.fromarray(wav_filter_image)
    ...
    # define a function that does `plot_every` steps before returning the 
    # sample for plotting
    ...
    print 'Creating a space to store the image...'
    # create a space to store the image for plotting ( we need to leave
    # room for the tile_spacing as well)
    # image_data = numpy.zeros((29*n_samples+1,29*n_chains-1),dtype='uint8')
    image_data = numpy.zeros(((1+freq_rows)*n_samples+1,
                              (1+timebins_cols)*n_chains-1),
                             dtype='uint8')
    wav_dataset = []
    for idx in xrange(n_samples):
        ...
        for ich in xrange(n_chains):
            ...
            pylab.plot(wdata, 'b')
            fname = 'wave_generative_%d_%d' % (idx, ich)
    ...
    datafile = gzip.open('wave_data.pkl.gz', 'w')
    cPickle.dump(wav_dataset, datafile)
    datafile.close()    

    datafile = gzip.open('image_data.pkl.gz', 'w')
